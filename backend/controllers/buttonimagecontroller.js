const ButtonImage = require('../models/buttonImagemodel');

// Create Button
exports.createButton = async (req, res) => {
  try {
    const newButton = new ButtonImage(req.body);
...
exports.getAllButtons = async (req, res) => {
  try {
    const buttons = await ButtonImage.find();
...
exports.getButtonById = async (req, res) => {
  try {
    const button = await ButtonImage.findById(req.params.id);
...
exports.updateButton = async (req, res) => {
  try {
    const button = await ButtonImage.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (!button) return res.status(404).json({ message: 'Button not found' });
    res.status(200).json(button);
  } catch (err) {
    res.status(400).json({ error: err.message });
...
exports.deleteButton = async (req, res) => {
  try {
    await ButtonImage.findByIdAndDelete(req.params.id);
